    ...
    <div className="card-container">
      {data?.data?.length > 0 && data?.data?.map((item) => (
        <div key={item?.id} className="card">
          <div className="card-img" id='image' onClick={() => handleImageClick(`../images/${item?.attributes?.name}.jpg`, item?.attributes?.name)}>
            <img src={`../images/${item?.attributes?.name}.jpg`} alt={item?.attributes?.name} className="img-fluid" />
          </div>
          <Link to={`/coffeeDetails/${item?.id}`} key={item?.id} id={item?.id}>
